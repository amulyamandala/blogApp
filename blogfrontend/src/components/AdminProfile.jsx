  ...
  return (
    <div className={pageWrapper}>
      ...
      {loading ? (
        ...
        <div>
          <h2 className="text-xl font-semibold text-[#1d1d1f] mb-6">All Users</h2>
          {users.length === 0 ? (
            <p className={mutedText}>No users found.</p>
          ) : (
            <div className="space-y-4">
              {users.map((user) => (
                <div key={user._id} className="bg-[#f5f5f7] rounded-3xl p-5 border border-[#e8e8ed] flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div>
                    <p className="text-base font-semibold text-[#1d1d1f]">{user.firstName} {user.lastName}</p>
                    <p className={mutedText}>{user.email}</p>
                    <p className="text-sm text-[#6e6e73] mt-1">Role: {user.role}</p>
                    <p className="text-sm mt-1">
                      Status: <span className={user.isActive ? "text-[#248a3d]" : "text-[#cc2f26]"}>{user.isActive ? "Active" : "Blocked"}</span>
                    </p>
                  </div>
                  <button
                    className={ghostBtn}
                  ...
                  >
                    {updatingUserId === user._id ? "Updating..." : user.isActive ? "Block user" : "Activate user"}
                  </button>
                </div>
              ))}
            </div>
        ...
        <div>
          <h2 className="text-xl font-semibold text-[#1d1d1f] mb-6">All Articles</h2>
          {articles.length === 0 ? (
              ...
              {articles.map((article) => (
                <div className={articleCardClass} key={article._id}>
                  <div>
                    ...
                    <p className="text-sm text-[#6e6e73] mt-2">{article.content.slice(0, 100)}...</p>
                  </div>
